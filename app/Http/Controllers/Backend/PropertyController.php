...
class PropertyController extends Controller
{
    ...
    {
        // Fetch all Property records from the database, ordered by the latest created ones.
        $property = Property::latest()->get();

        // Return a view named 'backend.property.all_property' and pass the fetched properties to the view.
    ...
    {
        // Retrieve the property ID from the incoming request
        $property_id = $request->id;

        // Find the Property model with the given ID and update its status to 0 (inactive)
        Property::findOrFail($property_id)->update([
            'status' => 0,
        ]);

        // Prepare a notification message for a successful property deactivation
        $notification = [
    ...
    {
        // Extracting property ID from the incoming request
        $property_id = $request->id;

        // Finding the Property model instance by its ID and updating its 'status' field to 1
        Property::findOrFail($property_id)->update([
            'status' => 1,
        ]);

        // Creating a notification array with a success message and alert type
        $notification = [
